...
import java.util.concurrent.atomic.AtomicInteger; // Add import for AtomicInteger
import java.util.stream.Collectors;

public class LiftRideClientProfilingPerformance {

    private static final String SERVER_URL = "http://localhost:8080/skiers/123/seasons/2024/days/6900/skiers/12345"; // Ensure this is correct
    private static final int NUM_THREADS = 32;
    private static final int NUM_REQUESTS_PER_THREAD = 100;
    ...
    private static List<RequestRecord> executeRequests() {
        ...
        AtomicInteger totalRequests = new AtomicInteger(0); // Track total requests sent by all threads

        for (int i = 0; i < NUM_THREADS; i++) {
            executor.submit(() -> {
                try {
                    long threadSuccessfulRequests = sendRequests(records, totalRequests); // Pass totalRequests for tracking
    ...
    private static long sendRequests(List<RequestRecord> records, AtomicInteger totalRequests) {
        ...
        while (totalRequests.getAndIncrement() < 10000) { // Limit total requests to 1000
                ...
                try {
                    ...
                    if (response.statusCode() == 201) {
                        ...
                        break;
						  ...
						  else { // Handle non-201 response codes here
						  System.out.println("Request failed from thread " + threadName +
						  ", Response code: " + response.statusCode()); break; }
    ...
    private static void calculateMetrics(List<RequestRecord> records, String filename) {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(filename))) {
            ...
            long totalTime = records.isEmpty() ? 0 : (records.get(records.size() - 1).getStartTime() - records.get(0).getStartTime());
            writer.write("Total time taken: " + totalTime + " milliseconds\n");
            System.out.println("Total response time: " + totalTime + " milliseconds");
            
            double throughput = 10000 / (totalTime / 1000.0); // Requests per second
